...
echo "== Grundgeruest"
for path in /health /signage/ /lage/ /stoerungen/ /wall/ /config/pages.txt /data/probe.json /woche/ /news/ /grafana/api/health; do
  c="$(code "${BASE}${path}")"
  case "${c}" in
    ...
    404) echo "  ${R}FEHLER${D} ${path} -> 404"
         case "${path}" in
           ...
           /grafana/*)        echo "         Grafana antwortet nicht. Der erste Start dauert"
                              echo "         laenger - er laedt die Plugins herunter. Pruefen:"
                              echo "           docker compose logs --tail 30 grafana" ;;
         esac
         FAILED=1 ;;
